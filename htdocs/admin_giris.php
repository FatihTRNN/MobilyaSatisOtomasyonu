...
if (isset($_POST['kullanici_adi']) && isset($_POST['sifre'])) { 
    ...
    if (!empty($kullaniciAdi) && !empty($sifre)) { 
        ...
        if ($sonuc->num_rows > 0) {
            ...
            header("Location: urun_islemleri.php"); // Girişten sonra yönlendirme
        } else {
            $mesaj = "Kullanıcı adınız veya şifreniz yanlış!";
        }
    } else {
        $mesaj = "Kullanıcı adı ve şifre girilmelidir."; 
    }
}
?>
...
<body>
    <div class="giris">
        <h1>Mobilya Center Admin Girişi</h1>
        <form method="post">
            ...
            <button type="submit" class="btn btn-primary btn-block btn-large" name="giris">Giriş Yap</button>
        </form>
        <a href="giris.php" class="btn btn-primary btn-block btn-large">Girişe Dön</a>
        <?php if (!empty($mesaj)): ?>
            <div class="alert" style="color: black;"><?php echo $mesaj; ?></div>
        <?php endif; ?>
    </div>
</body>
</html>

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_kontrol = "SELECT * FROM Kullanicilar WHERE kullanici_adi = ? AND sifre = ?";
    $stmt = $conn->prepare($sql_kontrol); // statement=hazırlık
    $stmt->bind_param("ss", $kullanici_adi, $sifre); // 2 string ifadeyi temsil ediyo "ss"
    $stmt->execute();
    ...
    if ($sonuc->num_rows > 0) {
        ...
        if ($stmt->execute() === TRUE) {
            // Kayıt başarılı olduğunda "Kayıt başarılı" mesajını gösterip giriş sayfasına yönlendir
            echo "<script>alert('Kayıt başarılı! Giriş ekranına yönlendiriliyorsunuz.');</script>";
